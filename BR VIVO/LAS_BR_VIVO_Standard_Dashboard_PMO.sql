// Standard Dashboard - BR VIVO Report

create or replace view LAS_BR_VIVO_Standard_Dashboard_PMO as
...
select
    ...
    ReplaceBlankNoDatesField(Site_Source.UF) "UF",
    ReplaceBlankNoDatesField(ExtensionObject_Source.Register_ID) "RegistroID",
    ReplaceBlankNoDatesField(ExtensionObject_Source.Validacao_do_Registro) "Validacao",
    ReplaceBlankNoDatesField(ExtensionObject_Source.Prolog) "Prolog",
    ReplaceBlankNoDatesField(Form_Items_Source.Logico) "Logico",
    ...
    ReplaceBlankNoDatesField(Form_Items_Source.Apply_Site_Survey) "Apply Site Survey",
    cast(Activity_Source.Projeto_Executivo_Real as date) as "Executive Plan Real",
    ...
    cast(Activity_Source.TX_Plan as date) as "TX Plan",
    ...
    from(
        ...
        where "ProjectCustomerName" = 'Vivo Brasil'
        ) as Project_Source
    ...
    left join(
        ...
        select
            ...
            "SiteName",
            "SiteRegion" UF,
            "SiteSubRegion" Regional
        from ID_MELA_DM_USR.SITE_GST_MELA_NATIVE_VIEW
        where "SiteCountry" = 'Brazil' and "ProjectCustomerIdCrg1" = '911810'
    ) as Site_Source
    ...
    left join(
            ...
            select
                ...
                from(
                    ...
                    select
                        ...
                        max(case when "ActivityName" = 'TX Availability' then FixDBDates("ActivityForecastActual") end) as TX_F,
                        ...
                        max(case when "ActivityName" = 'Projeto Executivo' then FixDBDates("ActivityActualDate") end) as PE_R
        ...
        left join(
            ...
            from(
                ...
                max(case when "ChecklistItemDescription" = 'Atividade Lógica' then "ChecklistItemValue" end) as Logico,
                max(case when "ChecklistItemDescription" = 'Classificação do Site' then "ChecklistItemValue" end) as Ofensor,
                max(case when "ChecklistItemDescription" = 'Prioridade' then "ChecklistItemValue" end) as Fisico2,
                max(case when "ChecklistItemDescription" = 'Site Nominal' then "ChecklistItemValue" end) as Fisico,
                max(case when "ChecklistItemDescription" = 'Gap' then "ChecklistItemValue" end) as Gap,
                max(case when "ChecklistItemDescription" = 'Vistoria N/A' then "ChecklistItemValue" end) as Apply_Site_Survey,
...
select * from LAS_BR_VIVO_Standard_Dashboard_PMO;
