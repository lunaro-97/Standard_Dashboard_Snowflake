// Standard Dashboard - BR CLARO Report - Physical
create or replace view LAS_BR_CLARO_SD_Physical_Ref as (

    with LAS_BR_CLARO_SD_Filtering as(
        select
        *
    ...
    , LAS_BR_CLARO_SD_MaxDates as (
        ...
        from LAS_BR_CLARO_SD_Filtering
        group by "Tools", "Scope", "Business Type", "Country", "Customer", "Fisico2", "Unique Site"
    )

    , LAS_BR_CLARO_SD_MaxDates_Final as(
        select 
        "Tools",
        "Scope",
        "Business Type",
        ...
        from LAS_BR_CLARO_SD_MaxDates
        group by "Tools", "Scope", "Business Type", "Country", "Customer", "Fisico2", "Unique Site"
    )
    
    , LAS_BR_CLARO_SD_MaxDates_Final_Unpvt as (
    ...
    "Max Milestones",
    "Max Date"
    from LAS_BR_CLARO_SD_MaxDates_Final
    ...
    unpivot(
        ...
        ) as LAS_BR_CLARO_SD_MaxDates_unpivot
    )
    
    select

        BR_CLARO_Final_Pivot."Tools",
        BR_CLARO_Final_Pivot."Scope",
        BR_CLARO_Final_Pivot."Business Type",
        BR_CLARO_Final_Pivot."Country",
        BR_CLARO_Final_Pivot."Customer",
        BR_CLARO_Final_Pivot."Unique Site",
        BR_CLARO_Final_Pivot."Max Milestones",
        BR_CLARO_Final_Pivot."Max Date"
        from
        (
            select
            "Tools",
            ...
            "Max Date",
                ...
                case
                    when 
                    (
                        "Max Milestones" not like 'Integration%'
                        and "Max Milestones" not like 'On Air%'
                        and "Max Milestones" not like 'Accepted%'
                    ) 
                    and "Fisico2" like '%RESSINT%' then '_out_of_scope'
                    else "Fisico2"
                end as "Logical Flag"
            
            from LAS_BR_CLARO_SD_MaxDates_Final_Unpvt
            where "Logical Flag" <> '_out_of_scope'
        ) as BR_CLARO_Final_Pivot
);

// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

select * from LAS_BR_CLARO_SD_Physical_Ref;
